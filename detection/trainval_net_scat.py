...
from scattering_parallel import DataParallelScat
    ...
    def forward(self, inputs,im_info,gt_boxes,num_boxes,Ms,Ns):
        ...
        for i, tensor in enumerate(tensors):
            with torch.cuda.device(tensor.get_device()):
                tensors[i] = tensors[i].view(tensors[i].size(0),tensors[i].size(1)*tensors[i].size(2),
                                             tensors[i].size(3),tensors[i].size(4))
                tensors[i] = tensors[i][:, :, :Ms, :Ns]
                tensors[i] = tensors[i].contiguous()
                tensors[i] = Variable(tensors[i])
                out.append([tensors[i],im_info[i].cuda(),gt_boxes[i].cuda(),num_boxes[i].cuda()])


        return out#tensors,im_info, gt_boxes, num_boxes

    def __call__(self, inputs,im_info,gt_boxes,num_boxes,Ms,Ns):
        return self.forward(inputs,im_info,gt_boxes,num_boxes,Ms,Ns)
...
# edouard
  parser.add_argument('--save_dir', dest='save_dir',
                      help='directory to save models', default="~/detection/important_detection/data",
                      type=str)
...
if __name__ == '__main__':
  ...
  if args.resume:
    ...
    adjust_learning_rate(optimizer, args.lr_decay_gamma)#unbug
  ...
  if args.mGPUs:
    scat = ScatteringMultiGPU(K, K, 3, list(range(torch.cuda.device_count())))
    fasterRCNN = DataParallelScat(fasterRCNN)
  else:
  ...
  for epoch in range(args.start_epoch, args.max_epochs+1):
    ...
    for step in range(iters_per_epoch):
      ...
      #S_data_temp = scat(im_scat_buffer) ; Uncomment to use without the option mGPUs.
      im_data = scat(im_scat_buffer,im_info,gt_boxes,num_boxes,Ms,Ns)
      """S_data_temp=S_data_temp.view(S_data_temp.size(0),S_data_temp.size(1)*S_data_temp.size(2),S_data_temp.size(3),S_data_temp.size(4))




      S_data_temp = S_data_temp[:, :, :Ms, :Ns]

      S_data_temp = S_data_temp.contiguous()
      im_data = Variable(S_data_temp)
      im_data.data.resize_(S_data_temp[:,:,:Ms,:Ns].size()).copy_(S_data_temp[:,:,:Ms,:Ns])

      im_data = Variable(S_data_temp[:, :, :Ms, :Ns]).contiguous()
      ...
      rpn_loss_cls, rpn_loss_box, \
      RCNN_loss_cls, RCNN_loss_bbox, \
      rois_label = fasterRCNN(*im_data)#im_data, im_info, gt_boxes, num_boxes)
