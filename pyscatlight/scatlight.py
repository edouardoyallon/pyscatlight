...
class Scatlight(object):
    ...
    def __init__(self, M, N, J, pre_pad=False, jit=True):
        ...
        # Create the filters
        filters = filters_bank(self.M_padded, self.N_padded, J, cache='M'+str(M)+'J'+str(J)+'.cached_filters')
    ...
    def _pad(self, input):
        if(self.pre_pad):
            ...
            #print(output.size())
            #print(out_.size())
            #print(output.narrow(4, 0, 1).size())
            output.narrow(4, 0, 1).squeeze(dim=4).copy_(out_)
        return output
    ...
    def forward(self, input):
        ...
        self.current_batch_size = input.size(0)

        if(not hasattr(self,'batch_size')):
            self.batch_size = input.size(0)
            self.input = torch.zeros(self.batch_size, input.size(1),input.size(2),input.size(3)).cuda()
        elif(self.current_batch_size<self.batch_size):
            self.input[:self.current_batch_size,...].copy_(input)
            input = self.input
        elif(self.current_batch_size>self.batch_size):
            self.batch_size = input.size(0)
            self.input = torch.zeros(self.batch_size, input.size(1),input.size(2),input.size(3))
        ...
        for n1 in range(len(psi)):
            ...
            S[..., n, :, :].copy_(unpad(U_J_r))
            n = n + 1
        return S[:self.current_batch_size,...]
